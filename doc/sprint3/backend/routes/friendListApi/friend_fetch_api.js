...
router.post('/api/friend/fetch', (req, res) => {
    ...
    console.log("Server received: POST /api/friend/fetch");
    let uid = req.body.uid;
    const session = getSession();
    // Run the Neo4j query

    session.run(`
        MATCH (u:User {uid: $uid})-[:FRIENDS_WITH]->(f:User)-[:HAS_PROFILE]->(p:User_profile)
    ...
    .then(result => {
        ...
        res.status(200).json({ friends: properties });
    })
    .catch(error => {
